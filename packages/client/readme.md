...
export default withTina(MyApp, {
  apis: {
    forestry: new ForestryClient({
      ...
      // getTokenFn: undefined, // Instead of grabbing the token from local storage, we can specify how its access token is retreived. You might want to use this if you are fetching content server-side.
    }),
  },
  sidebar: true,
});
