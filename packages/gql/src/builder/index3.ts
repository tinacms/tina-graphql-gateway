...
export const builder: Builder = {
  ...
  schema: async ({ cache }: { cache: Cache }) => {
    ...
    await sequential(sectionsObjects, async (section) => {
      const sectionInterfaceName = `${friendlyName(section.label)}_Interface`;
      accumulator.push({
        kind: "InterfaceTypeDefinition",
        name: {
          kind: "Name",
          value: sectionInterfaceName,
        },
        interfaces: [],
        directives: [],
        fields: [
      ...
      await sequential(section.templates, async (templateString) => {
        const template = await cache.datasource.getTemplate(templateString);
        const dataName = await builder.documentDataObject({
          cache,
          template,
          returnTemplate: false,
        ...
        accumulator.push(
          gql.object({
            ...
            fields: [
              gql.fieldRequired({ name: "id", type: "ID" }),
